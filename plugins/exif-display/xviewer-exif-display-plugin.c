...
#include "xviewer-exif-display-plugin-settings.h"
...
G_DEFINE_DYNAMIC_TYPE_EXTENDED (XviewerExifDisplayPlugin, xviewer_exif_display_plugin,
                PEAS_TYPE_EXTENSION_BASE, 0,
                G_IMPLEMENT_INTERFACE_DYNAMIC(XVIEWER_TYPE_WINDOW_ACTIVATABLE,
                                        xviewer_window_activatable_iface_init))

static void
xviewer_exif_display_plugin_init (XviewerExifDisplayPlugin *plugin)
{
}

/* xviewer_util_make_valid_utf8 is not exported so it's duped here */
gchar *
_xviewer_util_make_valid_utf8 (const gchar *str)
{
	GString *string;
...
/* stolen from xviewer-properties-dialog.c*/
static void
xviewer_exif_set_label (GtkWidget *w, ExifData *exif_data, gint tag_id)
{
	...
	if (exif_data) {
		...
		if (tag_id == EXIF_TAG_DATE_TIME_ORIGINAL && buf_ptr)
			...
			label_text = _xviewer_util_make_valid_utf8 (buf_ptr);
	}
			    ...
			    gboolean tooltip)
{
	...
	xviewer_exif_set_label (widget, exif_data, exif_tag);
...
/* stolen from xviewer-properties-dialog and slightly modified
 * you must g_free () the gchar* that I return!
 * */
static gchar*
xviewer_exif_get_focal_length_desc (ExifData *exif_data)
{
...
/* stolen from xviewer-properties-dialog and modified */
static void
xviewer_exif_set_focal_length_label (GtkWidget *w, ExifData *exif_data)
{
	gchar *focal_length_desc = xviewer_exif_get_focal_length_desc (exif_data);
	gtk_label_set_text (GTK_LABEL (w), focal_length_desc);

	g_free (focal_length_desc);
}

static void manage_exif_data (XviewerExifDisplayPlugin *plugin)
{
	XviewerImage *image;
	ExifData *exif_data;

	image = xviewer_thumb_view_get_first_selected_image (plugin->thumbview);
	...
	xviewer_exif_set_focal_length_label (GTK_WIDGET (gtk_builder_get_object (
...
static void manage_exif_data_cb (XviewerJob *job, gpointer data)
{
	if (!job->error) {
		manage_exif_data (XVIEWER_EXIF_DISPLAY_PLUGIN(data));
	}
}

static gboolean
calculate_histogram (XviewerExifDisplayPlugin *plugin, XviewerImage *xviewer_image)
{
	int rowstride;
	int width, height;
	int row, col;
	...
	image_pixbuf = xviewer_image_get_pixbuf (xviewer_image);
...
drawing_area_draw_cb (GtkDrawingArea *drawing_area, cairo_t *cr,
		     XviewerExifDisplayPlugin *plugin)
{
	gboolean draw_channels_histogram, draw_rgb_histogram;
	XviewerImage *xviewer_image;
	...
	xviewer_image = xviewer_thumb_view_get_first_selected_image (plugin->thumbview);
	g_return_if_fail (xviewer_image != NULL);
	...
	if (plugin->histogram_values_red == NULL) {
		 ...
		 * histogram again... */
		calculate_histogram (plugin, xviewer_image);
	}
	...
	g_object_unref (xviewer_image);
}

static void calculate_histogram_cb (XviewerJob *job, gpointer data)
{
	XviewerExifDisplayPlugin *plugin = XVIEWER_EXIF_DISPLAY_PLUGIN (data);

	if (!job->error) {
		XviewerImage *xviewer_image =
			xviewer_thumb_view_get_first_selected_image (plugin->thumbview);
		calculate_histogram (plugin, xviewer_image);
		g_object_unref (xviewer_image);
...
static void
statusbar_update_exif_data (GtkStatusbar *statusbar, XviewerThumbView *view)
{
	XviewerImage *image;
	ExifData *exif_data;
	...
	if (exif_data) {
		...
		focal_length = xviewer_exif_get_focal_length_desc (exif_data);
...
static void
selection_changed_cb (XviewerThumbView *view, XviewerExifDisplayPlugin *plugin)
{
	XviewerImage *image;
	...
	if (!xviewer_image_has_data (image, XVIEWER_IMAGE_DATA_EXIF)) {
		...
		job = xviewer_job_load_new (image, XVIEWER_IMAGE_DATA_EXIF);
	...
	if (!xviewer_image_has_data (image, XVIEWER_IMAGE_DATA_IMAGE)) {
		...
		job = xviewer_job_load_new (image, XVIEWER_IMAGE_DATA_IMAGE);
		g_signal_connect (G_OBJECT (job), "finished",
...
static void
remove_statusbar_entry (XviewerExifDisplayPlugin *plugin)
{
	GtkWidget *statusbar = xviewer_window_get_statusbar (plugin->window);
...
static void
setup_statusbar_exif (XviewerExifDisplayPlugin *plugin)
{
	GtkWidget *statusbar = xviewer_window_get_statusbar (plugin->window);
...
{
	XviewerExifDisplayPlugin *plugin = XVIEWER_EXIF_DISPLAY_PLUGIN (activatable);
	...
	settings = g_settings_new (XVIEWER_EXIF_DISPLAY_CONF_SCHEMA_ID);

	thumbview = xviewer_window_get_thumb_view (window);
	plugin->thumbview = XVIEWER_THUMB_VIEW (thumbview);
	...
	sidebar = xviewer_window_get_sidebar (window);
	...
	xviewer_sidebar_add_page (XVIEWER_SIDEBAR (sidebar), "Details",
			      plugin->gtkbuilder_widget);
	gtk_widget_show_all (plugin->gtkbuilder_widget);

	g_settings_bind (settings, XVIEWER_EXIF_DISPLAY_CONF_CHANNELS_HISTOGRAM,
			 plugin, "draw-chan-histogram", G_SETTINGS_BIND_GET);
	g_settings_bind (settings, XVIEWER_EXIF_DISPLAY_CONF_RGB_HISTOGRAM,
			 plugin, "draw-rgb-histogram", G_SETTINGS_BIND_GET);
	g_settings_bind (settings, XVIEWER_EXIF_DISPLAY_CONF_EXIF_IN_STATUSBAR,
...
impl_deactivate	(XviewerWindowActivatable *activatable)
{
	...
	xviewer_sidebar_remove_page(XVIEWER_SIDEBAR (sidebar),
				plugin->gtkbuilder_widget);
...
static void
xviewer_exif_display_plugin_set_draw_chan_histogram (XviewerExifDisplayPlugin *plugin,
						 gboolean value)
{
...
static void
xviewer_exif_display_plugin_set_draw_rgb_histogram (XviewerExifDisplayPlugin *plugin,
						gboolean value)
{
...
static void
xviewer_exif_display_plugin_enable_statusbar (XviewerExifDisplayPlugin *plugin,
					  gboolean value)
{
...
static void
xviewer_exif_display_plugin_get_property (GObject    *object,
				      guint       prop_id,
				      GValue     *value,
				      GParamSpec *pspec)
{
	XviewerExifDisplayPlugin *plugin = XVIEWER_EXIF_DISPLAY_PLUGIN (object);
...
static void
xviewer_exif_display_plugin_set_property (GObject      *object,
				      guint         prop_id,
				      const GValue *value,
				      GParamSpec   *pspec)
{
	...
	{
	case PROP_DRAW_CHAN_HISTOGRAM:
		xviewer_exif_display_plugin_set_draw_chan_histogram (plugin,
						g_value_get_boolean (value));	
		break;
	case PROP_DRAW_RGB_HISTOGRAM:
		xviewer_exif_display_plugin_set_draw_rgb_histogram (plugin,
						g_value_get_boolean (value));
		break;
	case PROP_ENABLE_STATUSBAR:
		xviewer_exif_display_plugin_enable_statusbar (plugin,
						g_value_get_boolean (value));
		break;
	case PROP_WINDOW:
...
static void
xviewer_exif_display_plugin_dispose (GObject *object)
{
	XviewerExifDisplayPlugin *plugin = XVIEWER_EXIF_DISPLAY_PLUGIN (object);
	...
	G_OBJECT_CLASS (xviewer_exif_display_plugin_parent_class)->dispose (object);
}
static void
xviewer_exif_display_plugin_class_init (XviewerExifDisplayPluginClass *klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS (klass);

	object_class->dispose = xviewer_exif_display_plugin_dispose;
	object_class->set_property = xviewer_exif_display_plugin_set_property;
	object_class->get_property = xviewer_exif_display_plugin_get_property;
...
static void
xviewer_exif_display_plugin_class_finalize (XviewerExifDisplayPluginClass *klass)
{
...
{
        xviewer_exif_display_plugin_register_type (G_TYPE_MODULE (module));
        peas_object_module_register_extension_type (module,
                                                    XVIEWER_TYPE_WINDOW_ACTIVATABLE,
                                                    XVIEWER_TYPE_EXIF_DISPLAY_PLUGIN);

	xviewer_exif_display_plugin_setup_register_types (module);
}
